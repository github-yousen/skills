...
- `--fetch N`：对前 N 个结果抓取正文并转 markdown（0=不抓，依赖 `http_client`）
- `--max-body-chars`：--fetch 模式下单个正文最大字符数，默认 5000（按段落边界截断）

**优势相比 PS 脚本**：自动解压 gzip（避免乱码）、反爬页检测（CF/百度安全验证等给出明确提示）、SSL 跳过、并行抓取（5 引擎并发）、JSON/Markdown 输出、snippet 提取更完整（PS 脚本只有 Bing 有 snippet）、可一键 `--fetch` 抓正文。

#### `--fetch` 模式：拿到搜索结果后直接读正文

**适用场景**：snippet 信息不足、跨多个结果综合、需要"读完才回答"。

```bash
# 搜 Python 异步编程 + 抓前 2 条正文（每条 max 5000 chars）
py scripts/web_search.py "Python 异步编程" --limit 5 --fetch 2 --max-body-chars 5000

# 配合域过滤：只搜官方文档并抓全文
py scripts/web_search.py "asyncio tutorial" --allowed-domains "docs.python.org" --fetch 3

# JSON 输出含 body 字段
py scripts/web_search.py "Vue3 Composition API" --fetch 2 --format json
```

**结果里 body 字段的状态**：
- `ok` —— 抓取成功
- `blocked(原因)` —— 命中反爬（CF 盾/百度验证/微博跳转等），正文**可能不完整**
- `fetch_failed(原因)` —— 网络/超时/空响应
- `error(类型)` —— 其他异常

**为什么需要 `--fetch`**：snippet 只是搜索引擎的缓存摘要，常见问题：
- 搜"Python 3.14 新特性" → snippet 只有 "Python 3.14.5 文档" 一行
- snippet 可能滞后
- 跨页综合信息时单条 snippet 拼接不出全貌
- 抓取 B 站/知乎/微博等有反爬的站点时，snippet 是 meta description 误导
...
```
                    --fetch N 触发
                          │
                          ▼
search_all(...)  ──→  [Result, Result, ...]
                          │ fetch_bodies(results, n=N)
                          ▼
              ┌───── for top-N results ─────┐
              │                              │
              ▼                              ▼
       Result #1  ──────→  fetch_url(r.url, return_meta=True)
       Result #2  ──────→        │
       ...                       │ (http_client.py)
                                ▼
                     _decompress (gzip/deflate/br)
                        _detect_charset (BOM→HTTP头→meta→域名兜底GBK)
                          detect_block (反爬特征)
                                │
                                ▼
                     HTML 字符串
                                ...
                                │ ├─ 块级 h1-h6/p/ul/ol/table 换行
                                │ ├─ 行内 a[href]→[text](url), b/i/code
                                │ └─ _cleanup 折叠空白
                                ▼
                     Markdown 字符串
                                │
                                ▼ _truncate_by_paragraph(md, max_chars)
                                │ (在最近的 \n\n 边界切断, 避免句中切断)
                                ▼
                     result.body = md
                     result.body_status = "ok" / "blocked(…)" / "fetch_failed(…)"
```

**关键设计**：
- **`fetch_url` 不抛异常**（所有错误都封装进返回 dict），所以 `fetch_bodies` 不会因为一条失败就中断
- **按段落边界截断**：在最近的 `\n\n` 切，避免在句子中间截断 LLM 看到半句话
- **状态写字段不抛异常**：LLM 看到 `body_status="blocked(...)"` 知道这条不可信，跳过即可
- **HTMLParser 自实现**：零外部依赖（不依赖 beautifulsoup/markdownify）
