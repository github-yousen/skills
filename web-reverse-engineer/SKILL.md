...
description: |
  网站逆向分析与站点技能生成。给定一个网站URL，通过抓取HTML/JS源码（而非浏览器渲染）逆向分析网站的接口、页面布局、鉴权流程和数据流。
  分析完成后可生成该网站的可复用操作技能，下次直接操作无需重新分析。
...
3. **沉淀**：产出分析文档或**站点技能**，下次直接复用，不用重新分析
...
| 用户说 | 进入模式 |
|--------|----------|
| "分析这个网站" / 给一个 URL | **分析模式**：理解功能 + 提取接口 + 产出文档，然后询问是否生成站点技能 |
| "帮我操作 xxx" + 已有凭证 | **操作模式**：直接调用接口完成任务（优先查找已有站点技能） |
| "帮我操作 xxx" + 没有凭证 | 先分析找到操作路径，提示用户提供凭证，完成后询问是否生成站点技能 |
| 给一个之前分析过的网站 | 优先读取已有的分析文档或站点技能，直接进入操作模式 |
...
分析报告生成后，如果 API 接口数 >= 1，询问用户：

> **分析完成！已生成 `xxx.com` 操作手册。是否要为这个网站生成一个可直接复用的操作技能？**
> 生成后下次只要说"帮我操作 xxx.com"即可直接调用，无需重新分析。

用户同意后，在本技能所在目录的上一级（skills 根目录）下生成 `site-{站点名}` 技能目录，与本技能同级：

```
skills/site-xxx/
├── SKILL.md
├── scripts/api_client.py
└── references/analysis_report.md
```

#### SKILL.md 生成规则

- `name`: `site-{站点域名（去除后缀）}`
- `description`: 通过 API 调用操作 {站点名} 的专用技能。当用户提到"操作"、"帮我xxx"、"用账号"等指令时触发。
- **描述中不要包含"分析"、"逆向"等触发词**，避免与 web-reverse-engineer 产生冲突。
- Body 部分写入：
  - **凭证说明**（从分析报告的凭证章节提取）
  - **API 调用模板**（从分析报告整理为可直接用的 Python 代码，参考本章"操作模式"中的请求构造模板）
  - **可直接执行的操作清单**（从报告第三章提取，标注方法、路径、必要参数）
  - **鉴权代码示例**（带 Cookie / Token 的调用示例）

#### scripts/api_client.py 生成规则
...
- 按模块分组生成函数（用户认证类、内容获取类、操作类等）
- 每个函数标注必要的参数说明
- 函数体为空或返回占位，具体参数由用户使用时传递
...
**生成完成后告知用户技能位置**：
> ✅ 已生成站点技能到 `skills/site-xxx/`。下次直接说"帮我操作 xxx.com"即可。
> 使用前需要进行一项操作：先在浏览器打开 xxx.com，F12 → Network 复制一条请求的 Cookie 填入 `api_client.py`。
